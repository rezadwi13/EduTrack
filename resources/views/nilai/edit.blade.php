            ...
            @method('PUT')
                ...
                <div>
                    ...
                    @foreach($siswas as $siswa)
                        <option value="{{ $siswa->id }}" data-kelas="{{ $siswa->kelas }}" @if(old('siswa_id', $nilai->siswa_id)==$siswa->id) selected @endif>{{ $siswa->nama }} ({{ $siswa->nis }}) - {{ $siswa->kelas }}</option>
                    @endforeach
                </select>
                ...
                <div>
                    ...
                    @foreach($mataPelajarans as $mataPelajaran)
                        <option value="{{ $mataPelajaran->id }}" @if(old('mata_pelajaran_id', $nilai->mata_pelajaran_id)==$mataPelajaran->id) selected @endif>{{ $mataPelajaran->nama }} ({{ $mataPelajaran->kode }}) - {{ $mataPelajaran->kelompok }}</option>
                    @endforeach
                </select>
                ...
                <div>
                    ...
                    <input type="number" name="nilai" value="{{ old('nilai', $nilai->nilai) }}" min="0" max="100" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                ...
                <div>
                    ...
                    <option value="1" {{ old('semester', $nilai->semester) == '1' ? 'selected' : '' }}>Semester 1</option>
                    <option value="2" {{ old('semester', $nilai->semester) == '2' ? 'selected' : '' }}>Semester 2</option>
                    <option value="3" {{ old('semester', $nilai->semester) == '3' ? 'selected' : '' }}>Semester 3</option>
                    <option value="4" {{ old('semester', $nilai->semester) == '4' ? 'selected' : '' }}>Semester 4</option>
                    <option value="5" {{ old('semester', $nilai->semester) == '5' ? 'selected' : '' }}>Semester 5</option>
                    <option value="6" {{ old('semester', $nilai->semester) == '6' ? 'selected' : '' }}>Semester 6</option>
                </select>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            filterKelas.addEventListener('change', function() {
                ...
                const currentSiswaOption = siswaSelect.querySelector(`option[value="${currentSiswa}"]`);
                const currentSiswaKelas = currentSiswaOption ? currentSiswaOption.getAttribute('data-kelas') : '';
                if (selectedKelas !== '' && currentSiswaKelas !== selectedKelas) {
                    siswaSelect.value = '';
                }
